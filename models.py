...
def UpBlock(intt, outt):
    block = nn.Sequential(
        nn.Upsample(scale_factor=2, mode='nearest'),
        #conv2d(in_planes, out_planes*2, 3, 1, 1, bias=False),
        # 线性可分离近似 3×3
        conv2d(intt, outt*2, (1,3), padding=(0,1), padding_mode='reflect'),
        conv2d(outt*2, outt*2, (3,1), padding=(1,0), padding_mode='reflect'),
        batchNorm2d(outt*2),GLU())
        
        #convTranspose2d(in_planes, out_planes*2, 4, 2, 1, bias=False),
    return block


def UpBlockComp(intt, outt):
    block = nn.Sequential(
        nn.Upsample(scale_factor=2, mode='nearest'),
        #conv2d(in_planes, out_planes*2, 3, 1, 1, bias=False),
        # 线性可分离近似 3×3
        ...
        batchNorm2d(outt*2), GLU(),
        # 线性可分离近似 3×3
        conv2d(outt, outt*2, (1,3), padding=(0,1), padding_mode='reflect'),
        conv2d(outt*2, outt*2, (3,1), padding=(1,0), padding_mode='reflect'),
        NoiseInjection(),
        batchNorm2d(outt*2), GLU()
        )
...
class Generator(nn.Module):
    def __init__(self, ngf=64, nz=100, nc=3, im_size=1024, use_grassmann=False):
        ...
        if im_size > 512:
         ...
         # ===== Grassmann branch (optional) =====
        if self.use_grassmann:
            self.cond16_to_4 = nn.Conv2d(nfc[16], nfc[4], 1, 1, 0, bias=True)
            self.cond16_to_8 = nn.Conv2d(nfc[16], nfc[8], 1, 1, 0, bias=True)
            self.cfg = cfg
            K = cfg.K

            # 让 GrassmannSeed 用同一份噪声 input（建议直接用 input）
            self.seed = GrassmannSeed(cfg.z_dim, K, rank=16, field_res=cfg.field_res)
            # 把 g0(K通道) -> nfc[128] 通道，用来注入 feat_128
            self.g0_to_16 = nn.Sequential(
                conv2d(cfg.K, nfc[16], 1, 1, 0, bias=False),
                batchNorm2d(nfc[16]),
                nn.LeakyReLU(0.1),
            )
    ...
    def forward(self, input):
        ...
        if self.use_grassmann:
            feat_64 = self.gcse_64(g0,self.feat_64(feat_32))
            feat_128 = self.gcse_128(g0,self.feat_128(feat_64))
        else:
        ...
        if self.use_grassmann:
            ...
            feat_256 = self.gcse_256(g0, self.feat_256(feat_128))
        else:
